...
class Persona extends CI_Model 
{
	...
	function exists($person_id)
	{
		...
		$this->db->where('personas.id_persona',$person_id);
		$query = $this->db->get();
	...
	*/
	...
	{
		$query = $this->db->get_where('personas', array('id_persona' => $person_id), 1);
	...
	*/
	...
	{
		$this->db->from('personas');
		$this->db->where_in('id_persona',$person_ids);
		$this->db->order_by("appaterno", "asc");
		return $this->db->get();		
	}
	...
	*/
	...
	{		
		...
		{
			if (isset($person_data['appaterno']) && isset($person_data['appaterno']))
			{
				$person_data['nombre_completo'] = $person_data['nombres'].' '.$person_data['appaterno'].' '.$person_data['apmaterno'];
			}
		
			if (!$person_id or !$this->exists($person_id))
			{
				$person_data['create_date'] = date('Y-m-d H:i:s');
				if ($this->db->insert('personas',$person_data))
				{
					$person_data['id_persona']=$this->db->insert_id();
					return true;
				}
			...
			$this->db->where('id_persona', $person_id);
			return $this->db->update('personas',$person_data);
		}
	...
	*/
	...
	function update_image($file_id,$person_id)
	{
		...
		$this->db->where('persona_id',$person_id);
