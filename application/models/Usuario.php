...
class Usuario extends Persona
{
	...
	*/
	function exists($person_id)
	{
		...
		$this->db->join('personas', 'personas.id_persona = usuario.id_persona');
		$this->db->where('usuario.id_persona',$person_id);
		$query = $this->db->get();
	...
	*/
	function get_all($deleted = 0,$limit=10000, $offset=0,$col='appaterno',$order='asc',$show_inactive=false,$location_id = '')
	{	
		if (!$deleted)
		{
			$deleted = 0;
		...
		$employees=$this->db->dbprefix('usuario');
		$employees_locations=$this->db->dbprefix('usuario_locations');
		$people=$this->db->dbprefix('personas');
		$data=$this->db->query("SELECT *,${people}.id_persona as pid 
						FROM ".$people."
						JOIN ".$employees." ON 										                       
						".$people.".id_persona = ".$employees.".id_persona
						JOIN ".$employees_locations." ON 										                       
						".$employees.".id_persona = ".$employees_locations.".employee_id
						WHERE deleted =$deleted and $inactive and $location GROUP BY $people.id_persona $order_by 
						LIMIT  ".$offset.",".$limit);		
	...
	function count_all($deleted=0,$location_id = '')
	{
		...
		$this->db->from('usuario');
		$this->db->join('usuario_locations','usuario_locations.id_usuario=usuario.id_usuario');	
		...
		$this->db->group_by('usuario.id_persona');
	...
	*/
	function get_info($employee_id, $can_cache = TRUE)
	{
		...
		$this->db->from('usuario');	
		$this->db->join('personas', 'personas.id_persona = usuario.id_persona');
		$this->db->where('usuario.id_persona',$employee_id);
		$query = $this->db->get();
   ...
   {
	    $this->db->select('p.*,u.usuario,u.estado');
	...
	*/
	function get_multiple_info($employee_ids)
	{
		$this->db->from('usuario');
		$this->db->join('personas', 'personas.id_persona = usuario.id_persona');		
		$this->db->where_in('usuario.id_persona',$employee_ids);
	...
	*/
	function get_multiple_locations_employees($location_ids)
	{
		...
		$this->db->from('usuario_locations');
	...
	function save_profile(&$person_data, &$employee_data, $employee_id)
	{
		...
		if(parent::save($person_data,$employee_id))
		{
			...
			{
				$employee_data['id_persona'] = $employee_id = $person_data['id_persona'];
				$success = $this->db->insert('usuario',$employee_data);
			}
			else
			{
				$this->db->where('id_persona', $employee_id);
				$success = $this->db->update('usuario',$employee_data);		
			}	
		}		
	...
	*/
	function save_usuario(&$person_data, &$employee_data,&$permission_data, &$permission_action_data, &$location_data=array(), $employee_id=false)
	{
		...
		if(parent::save($person_data,$employee_id))
		{
			...
			if($success)
			{
				...
				$success=$this->db->delete('permissions', array('id_persona' => $employee_id));
				...
				$success=$this->db->delete('permissions_actions', array('id_persona' => $employee_id));
				...
				$success=$this->db->delete('usuarios_locations', array('id_usuario' => $employee_id));
				...
				if($success)
				{
					if ($location_data !== FALSE)
					{
						...
						{
							$success = $this->db->insert('usuarios_locations',
							array(
	...
	*/
	...
	{
		$this->db->where('id_persona', $employee_id);
		return $this->db->update('usuario', array('deleted' => 1));		
	}
	...
	*/
	function delete_list($employee_ids)
	{
		...
		if(in_array($this->get_logged_in_employee_info()->id_persona,$employee_ids))
			return false;

			$this->db->where_in('id_persona',$employee_ids);
			return $this->db->update('usuario', array('deleted' => 1));
 	}
	...
	*/
	...
	{
		$this->db->where('id_persona', $employee_id);
		return $this->db->update('usuario', array('deleted' => 0));
	}
	...
	*/
	...
	function check_duplicate($term)
	{
		...
		$query = $this->db->where("appaterno = ".$this->db->escape($term));
		$query=$this->db->get();
	...
	*/
	...
	public function search($search, $rows = 0, $limit_from = 0, $sort = 'id_usuario', $order = 'asc', $count_only = FALSE)
	{
		// get_found_rows case
		$this->db->select('u.id_persona as id,u.usuario,p.nombre_completo,p.dni,p.email');
		$this->db->from('usuario u');
		$this->db->join('personas p', 'p.id_persona = u.id_persona');
		
		$this->db->where("(u.usuario LIKE '".$this->db->escape_like_str($search)."%' OR 
		p.appaterno LIKE '".$this->db->escape_like_str($search)."%' OR
		p.apmaterno LIKE '".$this->db->escape_like_str($search)."%' OR
		p.dni LIKE '".$this->db->escape_like_str($search)."%' OR
		p.nombre_completo LIKE '".$this->db->escape_like_str($search)."%') ");
		$this->db->where('p.deleted', 0);
	...
	{
		$this->db->select('u.id_usuario as id,u.usuario,p.nombre_completo,p.dni,p.email');
		$this->db->from('usuario u');
		$this->db->join('personas p', 'p.id_persona = u.id_persona');
	...
	*/
	...
	{
		//Username Query
		$query = $this->db->get_where('usuario', array('usuario' => $username, 'deleted'=> 0 ,'inactive' => 0), 1);
		if ($query->num_rows() ==1)
		{
			$row=$query->row();
			$this->session->set_userdata('id_persona', $row->person_id);
			return true;
		}
		return false;
	}
	...
	*/
	...
	{
		return $this->session->userdata('id_persona')!=false;
	}
	...
	*/
	...
	{ 
		...
		{
			$ret = $this->get_info($this->session->userdata('id_persona'));
	...
	*/
	function has_module_permission($module_id,$person_id)
	{
		...
		$query = $this->db->get_where('permissions', array('id_persona' => $person_id,'module_id'=>$module_id), 1);
	...
	function has_module_action_permission($module_id, $action_id, $person_id)
	{
		...
		$query = $this->db->get_where('permissions_actions', array('id_persona' => $person_id,'module_id'=>$module_id,'action_id'=>$action_id), 1);
	...
	function get_employee_by_username_or_email($username_or_email)
	{
		$this->db->from('usuario');	
		$this->db->join('personas', 'personas.id_persona = usuario.id_persona');
		$this->db->where('usuario',$username_or_email);
	...
	function cleanup()
	{
		$employee_data = array('usuario' => null);
		$this->db->where('deleted', 1);
		$this->db->update('usuario',$employee_data);
		$people_table = $this->db->dbprefix('personas');
		$app_files_table = $this->db->dbprefix('app_files');
		$employees_table = $this->db->dbprefix('usuario');
		$this->db->query('SET FOREIGN_KEY_CHECKS = 0');
		$this->db->query("DELETE FROM $app_files_table WHERE file_id IN (SELECT image_id FROM $people_table INNER JOIN $employees_table USING (id_persona) WHERE $employees_table.deleted = 1)");
		$this->db->query("UPDATE $people_table SET image_id = NULL WHERE id_persona IN (SELECT id_persona FROM $employees_table WHERE deleted = 1)");
	...
	function get_authenticated_location_ids($employee_id)
	{
		...
		$this->db->select('usuarios_locations.location_id');
		$this->db->from('usuarios_locations');
		$this->db->join('locations', 'locations.location_id = usuarios_locations.location_id');
		$this->db->where('id_usuario', $employee_id);
		$this->db->where('deleted', 0);
	...
	function is_location_authenticated($location_id)
	{
		if ($employee = $this->get_logged_in_employee_info())
		{
			...
			$this->db->from('usuarios_locations');
			$this->db->where('id_usuario', $employee->person_id);
			$this->db->where('location_id', $location_id);
			$result = $this->db->get();
	...
	function is_employee_authenticated($employee_id, $location_id)
	{
		...
		if (!$authed_employees)
		{
			...
			{
				$authed_employees[$employee['id_usuario']] = TRUE;
			}	
		}
