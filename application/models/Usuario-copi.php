...
class Usuario extends CI_Model {
	...
	*/
	public function get_info($employee_id)
	{
		...
		else
		{
			...
			foreach($this->db->list_fields('usuario') as $field)
			{
				$person_obj->$field = '';
			}
	...
	*/
	public function get_multiple_info($employee_ids)
	{
		$this->db->from('usuario');
		$this->db->join('personas', 'personas.id_persona = usuario.id_persona');
		$this->db->where_in('employees.id_persona', $employee_ids);
	...
	public function save_employee(&$person_data, &$employee_data, &$grants_data, $employee_id = FALSE)
	{
		...
		if(parent::save($person_data, $employee_id))
		{
			...
			if($success)
			{
				...
				$success = $this->db->delete('grants', array('id_persona' => $employee_id));
